 ...
 * @dev Uses Chainlink VRF 2.5 (uint256 subscriptionId)
 * 
 * Constructor Parameters (Base Mainnet):
 * - _usdc:           0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913
 * - _spin:           0x0e77F5Fd459d080EEc6C6A5cB903F66D2af1Cb07
 * - _admin:          YOUR_WALLET_ADDRESS
 * - _vrfCoordinator: 0xd5D517aBE5cF79B7e95eC98dB0f0277788aFF634
 * - _subscriptionId: 72032706697840176005689567060888708947863815104890830107786317680231688107309
...
contract MoltSpinRoulette is VRFConsumerBaseV2Plus, ReentrancyGuard {
    ...
    // Renamed to avoid conflict with Chainlink's owner
    ...
    constructor(
        ...
        if (_usdc == address(0) || _spin == address(0) || _admin == address(0)) revert ZeroAddress();
        USDC = IERC20(_usdc);
        SPIN = IERC20(_spin);
        admin = _admin;
    ...
    function withdrawUSDC(uint256 amt) external onlyAdmin {
        if (!USDC.transfer(admin, amt)) revert TransferFailed();
        emit HouseWithdrawal(admin, 0, amt);
    }
    
    function withdrawSPIN(uint256 amt) external onlyAdmin {
        if (!SPIN.transfer(admin, amt)) revert TransferFailed();
        emit HouseWithdrawal(admin, 1, amt);
    }
    
    // ============ ADMIN ============
    
    function setMinBetsUSDC(uint256 straight, uint256 even, uint256 dozen, uint256 max) external onlyAdmin {
    ...
    function setMinBetsSPIN(uint256 straight, uint256 even, uint256 dozen, uint256 max) external onlyAdmin {
