...
/**
 * @title MoltSpin - American Roulette on Base (V3 - Security Hardened)
 ...
 * - Fixed batch ID 0 bug (now starts at 1)
 * - Added timeout-based refund for stuck VRF requests
 * - Increased VRF callback gas limit
 * - Added ReentrancyGuard
 * - Added request validity tracking
 */
...
contract MoltSpinRouletteV3 is VRFConsumerBaseV2, ReentrancyGuard {
    // ============ CHAINLINK VRF ============
    
    VRFCoordinatorV2Interface public immutable VRF_COORDINATOR;
    uint64 public immutable subscriptionId;
    bytes32 public immutable keyHash;
    uint32 public constant CALLBACK_GAS_LIMIT = 500000; // Increased for 10 bets + transfers
    ...
    // Timeout for stuck VRF requests (1 hour)
    ...
    // VRF request tracking - uses struct for validity check
    struct VRFRequest {
        uint256 batchId;
        bool exists;
    }
    ...
    struct BetBatch {
        ...
        bool settled;
        bool refunded;     // Track if batch was refunded due to VRF timeout
        uint8 result;
    }
     ...
     */
    function placeBets(BetInput[] calldata betInputs) external whenNotPaused nonReentrant returns (uint256) {
        ...
        // Track VRF request with validity flag
    ...
    /**
     * @notice Refund a batch if VRF has not responded within timeout
     * @param batchId The batch to refund
     */
    function refundStuckBatch(uint256 batchId) external nonReentrant {
        ...
        // Mark all bets as settled (no payout)
        for (uint256 i = 0; i < batch.betIds.length; i++) {
            bets[batch.betIds[i]].settled = true;
        }
        
        // Refund the full bet amount
        if (!USDC.transfer(batch.player, batch.totalAmount)) revert TransferFailed();
        
        emit BatchRefunded(batchId, batch.player, batch.totalAmount, block.timestamp);
    }
    
    /**
     * @notice Check if a batch can be refunded
     * @param batchId The batch to check
     * @return canRefund Whether the batch can be refunded
     * @return timeUntilRefund Seconds until refund is available (0 if ready)
     */
