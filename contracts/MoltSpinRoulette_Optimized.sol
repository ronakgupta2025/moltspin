...
/**
 * ============================================================================
 * MOLTSPIN ROULETTE - OPTIMIZED FOR SIZE (< 24KB)
 * ============================================================================
 * 
 * Compile with: Optimizer ON, Runs: 1 (or 200)
 * 
...
contract MoltSpinRoulette is VRFConsumerBaseV2, ReentrancyGuard {
    ...
    uint256 private _redBitmap = 0x2A54A952A4; // Packed red numbers bitmap
    ...
    event BetPlaced(uint256 indexed betId, uint256 indexed batchId, address indexed player, uint8 token, uint8 betType, uint8 number, uint256 amount);
    event BatchCreated(uint256 indexed batchId, address indexed player, uint8 token, uint256 betCount, uint256 totalAmount, uint256 vrfRequestId);
    ...
    constructor(
        address _usdc, address _spin, address _owner,
        address _vrfCoordinator, uint64 _subscriptionId, bytes32 _keyHash
    ...
    function _placeBets(BetInput[] calldata inputs, uint8 token) internal returns (uint256) {
        ...
        for (uint256 i; i < len;) {
            BetInput calldata inp = inputs[i];
            ...
            bets[betId] = Bet(msg.sender, token, inp.betType, inp.number, 0, false, inp.amount, 0);
            betIds[i] = betId;
            total += inp.amount;
            maxPayout += _payout(inp.betType, inp.amount);
            
            emit BetPlaced(betId, batchId, msg.sender, token, inp.betType, inp.number, inp.amount);
            unchecked { ++i; }
        }
        ...
        batches[batchId] = BetBatch(msg.sender, token, 0, false, false, total, 0, block.timestamp, betIds);
        playerBatches[msg.sender].push(batchId);
        
        emit BatchCreated(batchId, msg.sender, token, len, total, reqId);
        return batchId;
    }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        uint256 payout;
        uint256[] storage ids = batch.betIds;
    ...
    function _isRed(uint8 n) internal view returns (bool) {
        if (n == 0 || n > 36) return false;
        // Red: 1,3,5,7,9,12,14,16,18,19,21,23,25,27,30,32,34,36
        uint256 mask = 1 << n;
        uint256 reds = (1<<1)|(1<<3)|(1<<5)|(1<<7)|(1<<9)|(1<<12)|(1<<14)|(1<<16)|(1<<18)|(1<<19)|(1<<21)|(1<<23)|(1<<25)|(1<<27)|(1<<30)|(1<<32)|(1<<34)|(1<<36);
        return (reds & mask) != 0;
    }
    
    function _checkWin(uint8 betType, uint8 num, uint8 res) internal view returns (bool) {
        if (betType == 0) return num == res; // Straight
        if (res == 0 || res == 37) return false; // 0/00 lose outside
    ...
    function _payout(uint8 betType, uint256 amt) internal pure returns (uint256) {
        if (betType == 0) return amt * 36;      // 35:1
        if (betType < 7) return amt * 2;        // 1:1
        return amt * 3;                          // 2:1
    }
    ...
    function setMinBetsUSDC(uint256 straight, uint256 even, uint256 dozen, uint256 max) external onlyOwner {
        minBetStraightUSDC = straight; minBetEvenMoneyUSDC = even; minBetDozenUSDC = dozen; maxBetUSDC = max;
    }
    
    function setMinBetsSPIN(uint256 straight, uint256 even, uint256 dozen, uint256 max) external onlyOwner {
        minBetStraightSPIN = straight; minBetEvenMoneySPIN = even; minBetDozenSPIN = dozen; maxBetSPIN = max;
    }
